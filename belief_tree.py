...
class BeliefNode(Node):
    ...
    """
    def __init__(self, nid, name, h, obs, parent=None, V=0, N=0, budget=float('inf')):
    ...
    def sample_state(self):
        ...
        idx=np.random.choice(np.linspace(0,len(self.B)-1,len(self.B)),p=self.belief)
...
class ActionNode(Node):
    ...
    """
    def __init__(self, nid, name, h, action, cost, parent=None, V=0, N=0):
        Node.__init__(self, nid, name, h, parent, V, N)
        self.mean_reward = 0.0
        self.mean_cost = 0.0
        self.cost = cost
    ...
    def add_child(self, node):
        self.children.append(node)
        self.obs_map[str(node.observation)] = node

    def get_child(self, observation):
        return self.obs_map.get(str(observation), None)
...
class BeliefTree:
    ...
    def add(self, h, name, parent=None, action=None, observation=None,
        ...
        if action is not None:
            n = ActionNode(self.counter, name, history, parent=parent, action=action, cost=cost)
        else:
            n = BeliefNode(self.counter, name, history, parent=parent, obs=observation, budget=budget)
